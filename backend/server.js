...
app.post('/api/v1/auth/login', (req, res) => {
  ...
  try {
    ...
    const employee = db.prepare('SELECT * FROM employees WHERE emp_id = ?').get(trimmedEmpId);
    if (!employee) {
...
app.get('/api/v1/employee/:emp_id', requireAuth, (req, res) => {
  const { emp_id } = req.params;
  try {
    const employee = db.prepare('SELECT emp_id, name, gender, position, department, email, phone FROM employees WHERE emp_id = ?').get(emp_id);
    if (!employee) {
...
app.post('/api/v1/employee/bulk-lookup', requireAuth, (req, res) => {
  ...
  try {
    ...
    const matched = db.prepare(query).all(cleanIds);
...
app.get('/api/v1/schedule', requireAuth, (req, res) => {
  try {
...
app.delete('/api/v1/schedule/:date_id', requireAdmin, (req, res) => {
  const { date_id } = req.params;
  try {
    db.prepare('DELETE FROM training_dates WHERE date_id = ?').run(date_id);
...
app.get('/api/v1/registration', requireAuth, (req, res) => {
...
app.post('/api/v1/registration/bulk-update', requireAuth, (req, res) => {
  ...
  try {
    ...
    for (const up of updates) {
      const reg = db.prepare('SELECT * FROM registrations WHERE reg_id = ?').get(up.reg_id);
      if (!reg) {
    ...
    if (!isAdmin) {
      for (const [dateIdStr, delta] of Object.entries(sessionChangeDeltas)) {
        ...
        if (delta > 0) {
          const session = db.prepare('SELECT title, max_capacity, (SELECT COUNT(*) FROM registrations WHERE date_id = ?) as current_count FROM training_dates WHERE date_id = ?').get(dateId, dateId);
          if (!session) {
    ...
    const updateStmt = db.prepare('UPDATE registrations SET date_id = ? WHERE reg_id = ?');
    
    const runTx = db.transaction((list) => {
      for (const item of list) {
        updateStmt.run(item.newDateId, item.reg.reg_id);
      }
    });
...
app.post('/api/v1/registration/bulk', requireAuth, (req, res) => {
  const user = req.session.user;
  ...
  try {
    ...
    const targetSession = db.prepare('SELECT title, status, max_capacity, (SELECT COUNT(*) FROM registrations WHERE date_id = ?) as current_count FROM training_dates WHERE date_id = ?').get(date_id, date_id);
    ...
    const runBulkTx = db.transaction((list) => {
      for (const item of list) {
        ...
        if (globalReg) {
          duplicateCount++;
          const empInfo = db.prepare('SELECT name FROM employees WHERE emp_id = ?').get(empId);
          duplicates.push(`${empInfo ? empInfo.name : empId}(이미 ${globalReg.title}에 등록됨)`);
          continue;
        }

        insertStatement.run(date_id, empId, user.emp_id, pCenter, sCenter);
        successCount++;
      }
    });
...
app.post('/api/v1/registration', requireAuth, (req, res) => {
  const user = req.session.user;
  ...
  try {
    ...
    const targetEmp = db.prepare('SELECT name FROM employees WHERE emp_id = ?').get(trimmedEmpId);
    if (!targetEmp) {
      return res.status(400).json({ error: '존재하지 않는 사번입니다.' });
    }

    const targetDate = db.prepare('SELECT status, max_capacity, (SELECT COUNT(*) FROM registrations WHERE date_id = ?) as current_count FROM training_dates WHERE date_id = ?').get(date_id, date_id);
    if (!targetDate) {
...
app.put('/api/v1/registration/:reg_id', requireAuth, (req, res) => {
  ...
  try {
    ...
    if (date_id && Number(date_id) !== reg.date_id) {
      const targetSession = db.prepare('SELECT title, max_capacity, (SELECT COUNT(*) FROM registrations WHERE date_id = ?) as current_count FROM training_dates WHERE date_id = ?').get(date_id, date_id);
      if (!targetSession) {
...
// Cancel registration
app.delete('/api/v1/registration/:reg_id', requireAuth, (req, res) => {
  const { reg_id } = req.params;
  const user = req.session.user;
